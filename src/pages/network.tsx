          ...
          <div className="grid sm:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              ...
              <motion.div
                ...
                className="bg-white border border-gray-200 shadow-sm rounded-2xl p-8 rounded-3xl border border-gray-200 hover:border-cyan-400/60 transition-all text-center group"
              >
                <div className="w-16 h-16 rounded-2xl bg-cyan-400/10 border border-cyan-400/30 flex items-center justify-center text-amber-700 mx-auto mb-4 group-hover:scale-110 transition-transform">
                  <MapPin className="w-8 h-8 text-amber-400" />
                </div>
                <span className="px-3 py-1 rounded bg-amber-400/20 text-amber-400 text-xs font-mono font-bold">{node.code}</span>
                <h3 className="text-2xl font-serif font-bold text-gray-950 mt-3 mb-2">{node.country}</h3>
                <p className="text-xs text-amber-700 font-mono mb-3">{node.cities}</p>
                <p className="text-xs text-gray-500 font-light border-t border-white/10 pt-3">{node.hub}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {connectedStakeholders.map((person, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-gray-50 border border-gray-200 flex items-center gap-3 hover:border-amber-400/50 transition-colors">
                <CheckCircle2 className="w-5 h-5 text-amber-400 shrink-0" />
                <span className="text-sm font-semibold text-gray-800">{person}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 relative">
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {perfectForList.map((item, idx) => (
              <motion.div
                ...
                className="glass-card p-6 rounded-2xl border border-gray-200 hover:border-cyan-400/50 transition-all duration-300"
              >
                <div className="w-12 h-12 rounded-xl bg-cyan-400/10 border border-cyan-400/30 flex items-center justify-center text-amber-700 mb-4">
          ...
          <div className="mt-16 text-center glass-card-gold rounded-3xl p-10 relative overflow-hidden">
            <h3 className="text-3xl font-serif font-bold text-gray-950 mb-4">Join the OneTrade360™ Global Network</h3>
